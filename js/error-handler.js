...
// Runtime DOM Integrity Check
(function runIntegrityCheck() {
  const path = window.location.pathname;
  const isDashboard = path.includes('admin-dashboard.html') || path.includes('dashboard.html');

  const required = {
    scripts: isDashboard ? ['api-config.js', 'dashboard.js'] : ['main.js'],
    elements: isDashboard ? ['#mainContent', '#sidebarNav', '#notificationBell'] : []
  };

  // If user dashboard, look for security-audit instead of dashboard.js
  if (path.includes('dashboard.html') && !path.includes('admin-dashboard.html')) {
    required.scripts = ['api-config.js', 'security-audit.js'];
  }

  // Check scripts loaded on window load to ensure sequential loading completed
  window.addEventListener('load', () => {
    const loadedScripts = Array.from(document.scripts).map(s => s.src);
    required.scripts.forEach(name => {
      if (!loadedScripts.some(src => src.includes(name))) {
        console.error(`[Integrity] Missing script: ${name}`);
      }
    });
  });
  ...
  window.addEventListener('DOMContentLoaded', () => {
    // 1. Enforce correct content image fallbacks and prevent logo.png placeholders
    function getCategoryFromPath() {
      const path = window.location.pathname.toLowerCase();
      if (path.includes('computer-laboratory')) return 'computer-laboratory';
      if (path.includes('science-laboratory')) return 'science-laboratory';
      if (path.includes('islamiyat-quran')) return 'islamiyat-quran';
      if (path.includes('kids-playground')) return 'kids-playground';
      if (path.includes('seminar-hall')) return 'seminar-hall';
      return 'facilities';
    }

    document.querySelectorAll('img').forEach(img => {
      const src = img.getAttribute('src') || '';
      // logo.png must ONLY be used for header, footer, official branding.
      if (src.includes('logo.png')) {
        const isBranding = img.closest('header') || img.closest('footer') || img.closest('.navbar-brand') || img.closest('.brand-logo') || img.closest('.logo-container') || img.classList.contains('site-logo') || (img.alt && (img.alt.toLowerCase().includes('school logo') || img.alt.toLowerCase().includes('official logo')));
        if (!isBranding) {
          const category = getCategoryFromPath();
          img.src = `assets/images/${category}/default.jpg`;
        }
      }
    });

    required.elements.forEach(selector => {
      if (!document.querySelector(selector)) {
        console.warn(`[Integrity] Missing element: ${selector}`);
      }
    });

    // Check all nav tabs have corresponding content panels
    document.querySelectorAll('[data-nav-target]').forEach(tab => {
      const target = tab.getAttribute('data-nav-target');
      if (!document.getElementById(target)) {
        console.error(`[Integrity] Nav tab "${tab.textContent.trim()}" points to missing panel: #${target}`);
      }
    });
  });

  // Global listener to capture and correct image load failures
  window.addEventListener('error', (event) => {
    const target = event.target;
    if (target && target.tagName === 'IMG') {
      const src = target.src || '';
      // Skip if already in branding areas
      const isBranding = target.closest('header') || target.closest('footer') || target.closest('.navbar-brand') || target.closest('.brand-logo') || target.closest('.logo-container') || target.classList.contains('site-logo');
      if (isBranding) return;

      // Map image path/alt to the correct local facility directory
      let category = 'facilities';
      const path = window.location.pathname.toLowerCase();
      if (path.includes('computer-laboratory') || src.includes('computer-laboratory') || (target.alt && (target.alt.toLowerCase().includes('computer') || target.alt.toLowerCase().includes('ict')))) {
        category = 'computer-laboratory';
      } else if (path.includes('science-laboratory') || src.includes('science-laboratory') || (target.alt && (target.alt.toLowerCase().includes('science') || target.alt.toLowerCase().includes('lab') || target.alt.toLowerCase().includes('microscope') || target.alt.toLowerCase().includes('glassware')))) {
        category = 'science-laboratory';
      } else if (path.includes('islamiyat-quran') || src.includes('islamiyat-quran') || (target.alt && (target.alt.toLowerCase().includes('quran') || target.alt.toLowerCase().includes('islamic') || target.alt.toLowerCase().includes('recitation')))) {
        category = 'islamiyat-quran';
      } else if (path.includes('kids-playground') || src.includes('kids-playground') || (target.alt && (target.alt.toLowerCase().includes('playground') || target.alt.toLowerCase().includes('slides') || target.alt.toLowerCase().includes('swing')))) {
        category = 'kids-playground';
      } else if (path.includes('seminar-hall') || src.includes('seminar-hall') || (target.alt && (target.alt.toLowerCase().includes('seminar') || target.alt.toLowerCase().includes('auditorium') || target.alt.toLowerCase().includes('speaker')))) {
        category = 'seminar-hall';
      }

      if (target.dataset.fallbackAttempted) {
        // If local default jpeg also failed, display a gorgeous responsive SVG with proper labels
        target.src = "data:image/svg+xml;charset=utf-8," + encodeURIComponent(`
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 500" width="100%" height="100%">
            <rect width="100%" height="100%" fill="#1e1b4b"/>
            <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="'Outfit', sans-serif" font-size="24" fill="#fbbf24" font-weight="bold">
              ${target.alt || 'Majestic School Facility'}
            </text>
          </svg>
        `);
        return;
      ...
      target.src = `assets/images/${category}/default.jpg`;
    }
  }, true);
})();
